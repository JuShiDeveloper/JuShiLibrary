...
public class MonthView extends ViewGroup {
    ...
    private View secondClickedView;//起止时间选择时,记录第二次点击的Item
    private List<StartStopBean> startStopDays = new ArrayList<>();//记录选择的起止时间
    ...
    public void setDateList(List<DateBean> dates, int currentMonthDays) {
        ...
        for (int i = 0; i < dates.size(); i++) {
            ...
            //找到起止时间选择时默认选中的日期，并选中（如果有）
            if (mAttrsBean.getChooseType() == 2 && mAttrsBean.getMultiDates() != null) {
                for (int[] d : mAttrsBean.getMultiDates()) {
                    if (date.getType() == 1
                            && d[0] == date.getSolar()[0]
                            && d[1] == date.getSolar()[1]
                        ...
                        setDayColor(view, COLOR_SET);
                        chooseDays.add(d[2]);
                        startStopDays.add(getStartStopBean(d[2], date, view));
                        break;
                    }
                }
                if (startStopDays.size() > 1) {
                    secondClickedView = startStopDays.get(1).getView();
                    secondClickedView.setTag(startStopDays.get(1).getDay());
                }
            }
            ...
            view.setOnClickListener(v -> {
                ...
                OnStartStopChooseListener startStopChooseListener = calendarView.getStartStopChooseListener();
                if (date.getType() == 1) {//点击当月
                    ...
                    } else if (mAttrsBean.getChooseType() == 2 && startStopChooseListener != null) { //选择起止时间模式
                        StartStopBean startStopBean = getStartStopBean(day, date, v);
                        if (chooseDays.contains(day)) {
                            setDayColor(v, COLOR_RESET);
                            chooseDays.remove(day);
                            startStopDays.remove(startStopBean);
                        } else {
                            setDayColor(v, COLOR_SET);
                            chooseDays.add(day);
                            startStopDays.add(startStopBean);
                        }
                        dealWithStartStopChoose(v, date, day, startStopChooseListener);
                    } else {
    ...
    private StartStopBean getStartStopBean(int day, DateBean date, View v) {
        StartStopBean bean = new StartStopBean();
        for (int i = 0; i < startStopDays.size(); i++) {
            if (startStopDays.get(i).getDay() == day) {
                return startStopDays.get(i);
            }
        }
        bean.setDateBean(date);
        bean.setDay(day);
        bean.setView(v);
        return bean;
    }
     ...
     * @param date
     * @param day
     * @param startStopChooseListener
     */
    private void dealWithStartStopChoose(View v, DateBean date, int day, OnStartStopChooseListener startStopChooseListener) {
        if (chooseDays.size() > 2) {
            Integer tag = (Integer) secondClickedView.getTag();
            if (chooseDays.contains(tag)) {
                chooseDays.remove(tag);
                setDayColor(secondClickedView, COLOR_RESET);
            }
            for (int i = 0; i < startStopDays.size(); i++) {
                if (startStopDays.get(i).getDateBean().getSolar()[2] == tag) {
                    startStopDays.remove(i);
                    break;
                }
            }
        }
        if (chooseDays.size() < 2) return;
        secondClickedView = v;
        secondClickedView.setTag(day);
        Collections.sort(startStopDays, new Comparator<StartStopBean>() {
            @Override
            public int compare(StartStopBean o1, StartStopBean o2) {
                return o1.getDay() < o2.getDay() ? -1 : 0;
            }
        });
        startStopChooseListener.onStartStopChoose(startStopDays.get(0), startStopDays.get(1));
    }
